...
ANGLES = [0.6, 0.8]
create_all(lattice = lattice, DIMS = DIMS, ANGLES = ANGLES, dir_name = 'small_angles', lammps = 'in.LONG_LV_CONDUCTIVITY')

ANGLES = [0.9, 1.0, 1.05, 1.08, 1.1, 1.2, 1.3, 1.5]
create_all(lattice = lattice, DIMS = DIMS, ANGLES = ANGLES, dir_name = 'big_angles', lammps = 'in.LV_CONDUCTIVITY')

# # test section
# DIMS = [(1, height) for height in range(1, 7 + 1)]
# ANGLES = [1.5]
# create_all(lattice = lattice, DIMS = DIMS, ANGLES = ANGLES, dir_name = 'test_section_1.50', lammps = 'in.LV_CONDUCTIVITY')

# # monolayer and bilayer
# width, height = 100, 60
# DIMS = [(width * n, height) for n in range(1, 7 + 1)]
# create_all(lattice = lattice, DIMS = DIMS, dir_name = 'monolayer', lammps = 'in.LV_CONDUCTIVITY')
